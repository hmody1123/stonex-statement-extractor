...
- Adds a classic daily-statement open-position backstop parser.
- Ensures continuation pages with only the `CONTRACT DESCRIPTION-OPEN` header are included.
- Fixes rubber/SCM daily statements so Open Positions includes rows across all open-position pages, not just page 1.
- Tested on `Daily Statement.05_12_2026 (1).pdf`: 206 raw open position rows parsed.

## v23 updates

- Realized PNL tab now falls back to summary-level REALIZED PROFIT & LOSS when no row-level realized PNL rows exist.
- Open Positions view exposes all columns in the column selector and selects all columns by default after the preferred columns.
- Removed the option to keep only the latest position snapshot; open positions are always appended across uploaded PDFs.
